...
export const RegistrationFormPrint: React.FC = () => {
  ...
  const handleDownloadPdf = async () => {
    ...
    const filename = formData?.user_data?.username
  ...
  }, [autoDownload, formData, isLoading, settingsLoading, paymentLoading]);

  if (isLoading || settingsLoading || paymentLoading) {
    return (
      <div className="min-h-screen bg-white p-8">
        <p className="text-center text-gray-600">Loading registration form...</p>
      </div>
    );
  }
  ...
  if (!formData) {
    return (
      <div className="min-h-screen bg-white p-8">
        <p className="text-center text-gray-600">Unable to load registration form.</p>
      </div>
    );
  }
